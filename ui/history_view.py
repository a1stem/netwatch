...
ui/history_view.py
------------------
Browse the SQLite connection history log.
Filters: All / Unknown / Blocked / By app name / Date range.
"""
...
class HistoryView(QWidget):
    ...
    def _apply_filter(self) -> None:
        ...
        if app_name:
            ...
            rows = self._history.query_recent(500)
        elif idx == 1:
            rows = self._history.query_unknown()
        elif idx == 2:
            rows = self._history.query_blocked()
        elif idx == 3:
            rows = self._history.query_since(datetime.now() - timedelta(hours=1))
        elif idx == 4:
            rows = self._history.query_since(datetime.now() - timedelta(hours=24))
        else:
            rows = self._history.query_recent(500)
        ...
        for r_idx, row in enumerate(rows):
            ...
            for col, val in enumerate(values):
                ...
                if is_blocked:
                    ...
                    item.setBackground(QBrush(QColor("#FDFDF8")))
